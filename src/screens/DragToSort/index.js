...
import { Image, Platform, StatusBar, Text, useWindowDimensions, View, StyleSheet} from "react-native";
...
function MovableSong({ id, artist, cover, title, positions, scrollY, songsCount }) {
    ...
    const gestureHandler = useAnimatedGestureHandler({
        ...
        onActive(event) {
            ...
            if (positionY <= scrollY.value + SCROLL_HEIGHT_THRESHOLD) {
                // Scroll up
                console.log("up", scrollY.value);
                scrollY.value = withTiming(0, { duration: 1000 });
                ...
                scrollY.value = withTiming(maxScroll, { duration: 1000 });
            } else {
                console.log("canceled", scrollY.value);
    ...
    return (
        <Animated.View style={animatedStyle}>
            <View style={{ maxWidth: "100%", position: "relative",backgroundColor:"white" }}>
                <Song artist={artist} cover={cover} title={title} />
                <PanGestureHandler onGestureEvent={gestureHandler}>
                    <Animated.View
                        style={{ position: "absolute", right: 30, top: SONG_HEIGHT / 2 - 10 }}
                    >
